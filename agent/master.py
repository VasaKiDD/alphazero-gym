...
class Agent:
    ...
    def play_game(self, exploit=False):
        ...
        obs = [obs, obs, obs, obs]
        observation = np.concatenate(tuple(obs), axis=-1)
        data = []
        end = False
        curr_reward = 0.0
        ...
        while not end:
            ...
            for i in range(len(actions)):
                ...
                episode["reward"] = 1.0 if rew > 0.0 else 0.0
    ...
    def train(
        ...
        for i in range(num_games):
            ...
            game_data[-1]["reward"] = -1.0
            self.general_data += game_data

            if not exploit:
                perm = np.arange(len(self.general_data))
                perm = np.random.permutation(perm)
                for j in tqdm(range(len(self.general_data))):
                    rewards = [
                        self.general_data[perm[j] - 1]["reward"]
                        if j > 0 and self.general_data[perm[j] - 1]["reward"] != -1.0
                        else 0.0
                    ]
                    actions = []
                    mcts_policies = []
                    forward_obs = []
                    for k in range(perm[j], len(self.general_data)):
                        rewards.append(self.general_data[k]["reward"])
                        actions.append(self.general_data[k]["action"])
                        mcts_policies.append(self.general_data[k]["mcts_policy"])
                        forward_obs.append(self.general_data[k]["state"])
                        if rewards[-1] == -1.0:
                            break
                    for k in range(1, len(rewards)):
                        decay = rewards[-k] * self.reward_decay
                        rewards[-1 - k] += decay
                        rewards[-1 - k] = 1.0 if rewards[-1 - k] > 1.0 else rewards[-1 - k]
                    cross_ent, val_loss, encod_loss = self.optimize_controller(
                        actions, mcts_policies, rewards, forward_obs[: self.forward_limit], True
                    )
                    print(
                        cross_ent,
                        val_loss,
                        float(encod_loss),
                        len(forward_obs[: self.forward_limit]),
                    )
